...
void getWords(char const *line, size_t lineBufferSize, std::unique_ptr<char *> words)
{
	size_t wordBufferSize = 100;
	for (int lineCharIndex{ 0 }, wordCharIndex{ 0 }; lineCharIndex < lineBufferSize && wordCharIndex < wordBufferSize; lineCharIndex++)
	{
...
void formatLine(char *line, size_t lineBufferSize)
{
	...
	for (int charIndex{ 0 }; charIndex < lineBufferSize; charIndex++)
	{
		...
		else if (line[charIndex] == ' ' || line[charIndex] == '\9')
		{
